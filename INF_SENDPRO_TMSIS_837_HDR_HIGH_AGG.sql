...
select RUN_DATE, MCO, Types, PRIORITY, MEASURE, Criteria, CASE WHEN SUM_ALL_REC_CNT = 0 THEN NULL ELSE SUM_VALID_REC_CNT/SUM_ALL_REC_CNT END AS PCT_VALID 
from (
select RUN_DATE, MCO, Types, PRIORITY, MEASURE, Criteria, SUM(VALID_REC_CNT) AS SUM_VALID_REC_CNT, 
SUM(ALL_REC_CNT) AS SUM_ALL_REC_CNT  
FROM (
select RUN_DATE, CDE_ENC_MCO AS MCO, 'Measure' AS Types, PRIORITY, MEASURE, BENCHMARK_THRESHOLD AS Criteria, Type, 
CASE WHEN TYPE IN ('VALID') THEN REC_CNT ELSE 0 END AS VALID_REC_CNT,
CASE WHEN TYPE IN ('VALID','NULL','INVALID') THEN REC_CNT ELSE 0 END AS ALL_REC_CNT
from MHTEAM.DWDQ.INF_SENDPRO_TMSIS_837_HDR_HIGH_AGG
)
GROUP BY RUN_DATE, MCO, Types, PRIORITY, MEASURE, Criteria
)
...
select 
    ...
    NULL AS Measures,
    MAX(CASE WHEN RUN_DATE = '2025-09-30' THEN PCT_VALID ELSE 0 END) AS Current_Status,    
...
from (
---
select RUN_DATE, MCO, CASE WHEN SUM_ALL_REC_CNT = 0 THEN NULL ELSE SUM_VALID_REC_CNT/SUM_ALL_REC_CNT END AS PCT_VALID,
from (
...
SELECT 
    ...
    SUM(CASE WHEN RUN_DATE = '2025-09-30' THEN PF_SUM ELSE 0 END) AS Current_Status,    
...
select
TO_DATE('20250930','YYYYMMDD') AS CUR_DATE,
MCO, 
